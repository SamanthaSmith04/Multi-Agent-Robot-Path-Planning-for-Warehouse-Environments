...
for i = 1:num_robots
    ...
    currently_occuppied(i) = robot_array(i).node_path(1);
end
...
while sum(running)
    ...
    for i=waiting_queue
        bot = robot_array(i);
        if(i==1 && currentTime + epsilon >= 53.500 && currentTime - epsilon <= 53.500 )
            disp(""); 
        end
        completed = 0; %Indicates if a robot has compelted its path
        % Attempt To Reserve Next k Nodes
        %Be espilon greater to avoid round errors
        if currentTime - epsilon > bot.plannedTime && running(i)
